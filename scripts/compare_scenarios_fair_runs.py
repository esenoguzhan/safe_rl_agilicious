...
Usage:
  python scripts/compare_scenarios_fair_runs.py \\
    --scenarios_config configs/config_scenarious_2.yaml \\
...
# Reuse rollout + env stack from compare_scenarios.py
...
def save_scenario_rollout_bundle(
    ...
    *,
    ...
    ``rollouts_by_seed`` entries match ``compare_scenarios._run_*_scenario`` return dicts
...
def main() -> None:
    ...
    parser.add_argument(
        "--scenarios_config",
        type=str,
        default=os.path.join(_REPO_ROOT, "configs", "config_scenarious_2.yaml"),
    )
    ...
    with open(log_path, "w") as f:
        f.write("compare_scenarios_fair_runs log\n")
