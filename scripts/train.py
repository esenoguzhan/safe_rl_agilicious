...
This script does not modify ``train_core.py``; it reuses its env factory and
constants via imports and mirrors its success / LR schedule / callbacks.

Usage:
  python scripts/train.py \\
      --config configs/ppo_config_base.yaml \\
...
→ (same chain as train_core: observation noise, custom reward, max steps, …).
"""
...
from scripts.train_core import (
    EVAL_SEED,
    SUCCESS_MIN_REWARD,
...
def main():
    parser = argparse.ArgumentParser(
        description="PPO (train_core schedule) with decimated high-rate Flightmare physics"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Resume from run directory or model .zip (same as train_core)",
    )
    ...
    print(
        f"[train] physics={ph:g} Hz (sim_dt={physics_dt:g} s), "
    ...
    if prev_sim_dt is not None and abs(float(prev_sim_dt) - physics_dt) > 1e-12:
        print(
            f"[train] YAML sim_dt was {prev_sim_dt}; overridden to {physics_dt:g}"
        )
    ...
    if env.command_delay_active:
        if randomize_command_delay:
            print(
                f"[train] Command delay enabled: nominal="
        ...
        def _on_step(self):
            ...
            if is_eval_step and self.evaluations_results and self.evaluations_length:
                ...
                if success_rate > self.best_success_rate:
                    ...
                    if self.verbose > 0:
                        print(
                            f"[train] Eval success rate {success_rate:.1%} > previous best "
                            f"{max(prev, 0.0):.1%} → saved best_model.zip"
                            + (" + vecnormalize.pkl" if self._vecnorm_cb is not None else "")
                        )
                elif self.verbose > 0:
                    print(
                        f"[train] Eval success rate {success_rate:.1%} ≤ best "
    ...
    class SuccessScheduleV3Callback(BaseCallback):
        ...
        def _on_step(self) -> bool:
            ...
            if target_lr != self._lr or target_ent != self._ent:
                ...
                if self.verbose > 0:
                    print(
                        f"[train] Eval success {rate:.1%} → band lr={self._lr:g} "
            ...
            if self._consecutive_at_stop_thr >= 3:
                print(
                    f"[train] Stopping: {self._consecutive_at_stop_thr} consecutive evals "
    ...
    print(
        "[train] Success = episode length "
        f"{success_episode_length} and return > {SUCCESS_MIN_REWARD}; "
        "same LR/entropy schedule as train_core."
    )
