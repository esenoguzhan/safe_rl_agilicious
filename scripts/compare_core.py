...
Usage:
  python scripts/compare_core.py \
    --scenarios_config configs/<your_scenarios>.yaml \
...
def main():
    ...
    parser.add_argument("--scenarios_config", type=str, default=None,
                        help="Path to a scenarios YAML (with a top-level 'scenarios:' list). Required.")
    ...
    if not args.scenarios_config:
        parser.error("--scenarios_config is required (path to a scenarios YAML).")
    ...
    log_path = os.path.join(log_dir, f"compare_core_log_{ts}.txt")
